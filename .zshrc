...
export ZSH="/home/pato05/.oh-my-zsh"
...
editas() {
    [ -z "$1" ] && echo "Usage: editas <file>" && return 1
    
    local BASENAME=`basename "$1"`
    local TMPNAME="/tmp/$BASENAME.$RANDOM"

    doas cat "$1" | cat > "$TMPNAME"
    $EDITOR "$TMPNAME"
    cat "$TMPNAME" | doas tee "$1" >/dev/null
}
